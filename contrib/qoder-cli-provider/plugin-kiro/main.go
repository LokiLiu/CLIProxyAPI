...
import (
	...
	"github.com/router-for-me/CLIProxyAPI/v7/contrib/qoder-cli-provider/internal/kiroprovider"
...
func cliproxyPluginCall(method *C.char, request *C.uint8_t, requestLen C.size_t, response *C.cliproxy_buffer) C.int {
	...
	if err != nil {
		writeResponse(response, errorEnvelope("plugin_error", err.Error(), false, http.StatusBadGateway))
		return 1
	}
...
func handleMethod(method string, request []byte) ([]byte, error) {
	...
	case pluginabi.MethodPluginRegister, pluginabi.MethodPluginReconfigure:
		return okEnvelope(registration{
			...
			Metadata:      pluginapi.Metadata{Name: "Kiro CLI Provider", Version: "0.1.0", Author: "LokiLiu", GitHubRepository: "https://github.com/LokiLiu/CLIProxyAPI"},
	...
	case pluginabi.MethodAuthLoginStart, pluginabi.MethodAuthLoginPoll:
		return errorEnvelope("login_unsupported", "Run kiro-cli login before adding its CLIProxyAPI auth JSON", false, http.StatusNotImplemented), nil
	...
	case pluginabi.MethodExecutorHTTPRequest:
		return errorEnvelope("unsupported", "Kiro provider does not expose arbitrary HTTP requests", false, http.StatusNotImplemented), nil
	default:
...
func parseAuth(raw []byte) ([]byte, error) {
	...
	account, handled, err := kiroprovider.ParseAccount(request.RawJSON, request.FileName)
	if err != nil {
		return nil, err
	}
	if !handled {
	...
	return okEnvelope(pluginapi.AuthParseResponse{Handled: true, Auth: authData(account, request.FileName)})
}
...
func authData(account kiroprovider.Account, fileName string) pluginapi.AuthData {
	return pluginapi.AuthData{
		Provider: pluginIdentifier, ID: account.ID, FileName: fileName, Label: account.Label, Prefix: account.Prefix,
		StorageJSON: append([]byte(nil), account.Raw...),
		Metadata:    map[string]any{"type": "kiro", "cli_path": account.CLIPath, "label": account.Label},
...
func modelsForAuth(raw []byte) ([]byte, error) {
	...
	models, err := kiroprovider.DiscoverModels(context.Background(), account)
	if err != nil {
		return nil, err
	}
	...
	for _, model := range models {
		...
		infos = append(infos, pluginapi.ModelInfo{
			ID: id, Name: id, Object: "model", OwnedBy: account.Prefix, DisplayName: model.Name,
			Description: model.Description, ContextLength: model.ContextWindow, InputTokenLimit: model.ContextWindow,
			SupportedGenerationMethods: []string{"chat"},
			SupportedParameters:        []string{"max_tokens", "stream", "system", "tools", "tool_choice"},
			SupportedInputModalities:   []string{"text"}, SupportedOutputModalities: []string{"text"}, UserDefined: true,
		})
	}
...
func execute(raw []byte) ([]byte, error) {
	...
	result, err := kiroprovider.Run(context.Background(), account, invocation, nil)
	if err != nil {
		return nil, err
	}
...
func runStream(streamID string, account kiroprovider.Account, invocation provider.Invocation) {
	id := fmt.Sprintf("chatcmpl_%x", time.Now().UnixNano())
	if err := emitStream(streamID, provider.EncodeStreamRole(id, invocation.Model)); err != nil {
		closeStream(streamID, err)
		return
	}
	result, err := kiroprovider.Run(context.Background(), account, invocation, func(text string) error {
		return emitStream(streamID, provider.EncodeStreamText(id, invocation.Model, text))
	})
	if err == nil {
		err = emitStream(streamID, provider.EncodeStreamFinish(id, result))
	}
	closeStream(streamID, err)
}

func executionInput(request pluginapi.ExecutorRequest) (kiroprovider.Account, provider.Invocation, error) {
	if request.Format != "" && request.Format != "openai" {
		return kiroprovider.Account{}, provider.Invocation{}, fmt.Errorf("unsupported executor format %q", request.Format)
	}
	account, err := accountFromStorage(request.StorageJSON, request.AuthID)
	if err != nil {
		return kiroprovider.Account{}, provider.Invocation{}, err
	}
	model := strings.TrimPrefix(strings.TrimSpace(request.Model), account.Prefix+"/")
...
func accountFromStorage(raw []byte, authID string) (kiroprovider.Account, error) {
	account, handled, err := kiroprovider.ParseAccount(raw, authID+".json")
	if err != nil {
		return kiroprovider.Account{}, err
	}
	if !handled {
		return kiroprovider.Account{}, fmt.Errorf("auth %q is not a Kiro account", authID)
	}
	return account, nil
}
